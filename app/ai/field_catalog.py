...
def format_field_catalog_prompt(catalog: list[dict] | None) -> str:
    ...
    return (
        ...
        "10) If a document label/heading matches a catalog field label (or clear synonym / OCR misspelling), that field is REQUIRED to be filled.\n"
        "11) Never leave readable document data unused when a catalog field exists for it.\n"
        "12) Never invent a value that is not in the document. Missing fields must be null/omitted.\n"
        "13) Policy Holder, Agent, and Beneficiary are different people — map each to the matching catalog field.\n"
        "14) DATE FIELDS (critical):\n"
        ...
        "   - If Effective, Expiration, and Renewal dates all appear, keep them distinct. Do not copy expiration into renewal unless the catalog has no expiry field.\n"
