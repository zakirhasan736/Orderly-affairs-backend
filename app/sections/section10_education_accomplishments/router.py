...
async def save_section10(
    ...
    for education_items in raw_data.values():
        for edu in education_items:
            for field in edu.model_dump().values():
...
@router.get("")
async def get_section10(authorization: str = Header(...)):
    token = authorization.split(" ")[1]
    decoded = verify_token(token)
...
@router.delete("")
async def delete_section10(authorization: str = Header(...)):
    token = authorization.split(" ")[1]
    decoded = verify_token(token)
