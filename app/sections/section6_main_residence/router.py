# app/sections/section6_main_residence/router.py
...
from app.sections.section6_main_residence.schemas import (
    Section6MainResidencePayload,
)

router = APIRouter(
    prefix="/sections/section6-main-residence",
    tags=["Section 6 – Main Residence"],
)
...
@router.post("")
async def save_section6(
    payload: Section6MainResidencePayload,
    ...
    data = {
        "6A": raw["6A"].model_dump(exclude_none=True)
    }

    # 🔥 Delete removed Cloudinary files
    for field in raw["6A"].model_dump().values():
    ...
    return {"message": "Section 6 saved successfully"}


@router.get("")
async def get_section6(authorization: str = Header(...)):
    token = authorization.split(" ")[1]
    decoded = verify_token(token)
...
@router.delete("")
async def delete_section6(authorization: str = Header(...)):
    token = authorization.split(" ")[1]
    decoded = verify_token(token)
    ...
    return {"message": "Section 6 deleted"}
